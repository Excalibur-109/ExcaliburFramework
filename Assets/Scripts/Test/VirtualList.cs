...
public class VirtualList : MonoBehaviour
{
    ...
    public Vector2 InstantiateSize
    {
        get
        {
            if (m_InstantiateSize == Vector2.zero)
            {
                ...
                {
                    rows = m_Page.x;                         //行数
                    cols = m_Page.y + (float)m_BufferNo;     //列数(+缓存的格子数)
                }
                else
                {
    ...
    public int InstantiateCount { get { return (int)InstantiateSize.x * (int)InstantiateSize.y; } }   //实际应该生成的格子数(行x列)
    ...
    public float scale { get { return direction == Direction.Horizontal ? 1f : -1f; } }    //当坐标系位于左上角时的相对位置
