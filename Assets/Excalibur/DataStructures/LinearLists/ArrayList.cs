...
namespace Excalibur.DataStructures
{
    ...
    public class ArrayList<T> : IEnumerable<T>
    {
        ...
        public readonly T[] _emptyArray = new T[0];
        ...
        public T this[int index]
        {
            get
            {
                if (_handleErrorOfIndex(index))
                {
                    throw new IndexOutOfRangeException("Please pass valid start index.");
                }
        ...
        {
            if (_handleErrorOfIndex(startIndex))
            {
                throw new IndexOutOfRangeException("Please pass valid start index.");
            }

            if (_handleErrorOfIndex(startIndex, count))
            {
                throw new ArgumentOutOfRangeException();
            }
        ...
        {
            if (_handleErrorOfIndex(startIndex)) // (startIndex < 0 || startIndex > _size)
            {
                throw new IndexOutOfRangeException("Please pass valid start index.");
            }

            if (_handleErrorOfIndex(startIndex, count)) // (count < 0 || startIndex > (_size - count))
            {
                throw new ArgumentOutOfRangeException();
            }
        ...
        private bool _handleErrorOfIndex(int index)
        {
            if (index < 0 || index >= _size) //(uint)startIndex > (uint)_size)
            {
                return true;
            }
        ...
        private bool _handleErrorOfIndex(int index, int count)
        {
